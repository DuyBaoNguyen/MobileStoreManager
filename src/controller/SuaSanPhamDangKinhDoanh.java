...
@WebServlet("/SanPhamDangKinhDoanh/SuaSanPham")
public class SuaSanPhamDangKinhDoanh extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public SuaSanPhamDangKinhDoanh() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			Boolean updateActProductError = (Boolean) getServletContext().getAttribute("updateActProductError");
			if (updateActProductError != null) {
				getServletContext().removeAttribute("updateActProductError");
			} else {
				updateActProductError = false;
			}
			request.setAttribute("updateActProductError", updateActProductError);

			request.getRequestDispatcher("/WEB-INF/SuaSanPhamDangKinhDoanh.jsp").forward(request, response);
		} catch (Exception e) {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			boolean updateActProductError = ProductDAO.updateProduct(product);
			if (updateActProductError == true) {
				getServletContext().setAttribute("updateActProductError", updateActProductError);
				response.sendRedirect(
						request.getContextPath() + "/SanPhamDangKinhDoanh/SuaSanPham?productId=" + product.getId());
			} else {
				response.sendRedirect(
						request.getContextPath() + "/SanPhamDangKinhDoanh/HangSanPham?manufId=" + manuf.getId());
			}
		} catch (Exception e) {
			response.sendRedirect(request.getContextPath() + "/SanPhamDangKinhDoanh");
		}
